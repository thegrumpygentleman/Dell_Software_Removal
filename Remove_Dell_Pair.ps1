# PowerShell Script to Force Uninstall Dell Pair Application
# Optimized for SYSTEM user context (Action1, SCCM, etc.)

Write-Host "Starting Dell Pair application removal process (SYSTEM context)..." -ForegroundColor Green
...
# Stop any running Dell Pair processes for all users
Write-Host "Stopping Dell Pair processes..." -ForegroundColor Yellow
$processes = @("DellPair", "Dell.Pair", "DellPairApp", "Dell Pair", "DellPair*")
...
try {
    $apps = Get-WmiObject -Class Win32_Product | Where-Object { 
        $_.Name -like "*Dell Pair*" -or 
        $_.Name -like "*DellPair*" -or 
        $_.Name -like "*Dell*Pair*"
    }
    foreach ($app in $apps) {
...
foreach ($basePath in $registryPaths) {
    try {
        ...
        foreach ($key in $uninstallKeys) {
            ...
            if ($displayName -like "*Dell Pair*" -or 
                $displayName -like "*DellPair*" -or
                $displayName -like "*Dell*Pair*") {
                    ...
                    try {
                        if ($uninstaller -like "*msiexec*") {
                            ...
                            if ($uninstaller -match "\{[A-F0-9\-]+\}") {
                                $msiCode = $matches[0]
                                $arguments = "/X$msiCode /quiet /norestart /L*v `"$env:TEMP\DellPair_Uninstall.log`""
...
$systemPaths = @(
    ...
    "$env:ProgramFiles\Dell Pair",
    "$env:ProgramFiles\DellPair",
    "${env:ProgramFiles(x86)}\Dell\Dell Pair",
    "${env:ProgramFiles(x86)}\Dell Pair",
    "${env:ProgramFiles(x86)}\DellPair",
    "$env:ProgramData\Dell\Dell Pair",
    "$env:ProgramData\Dell Pair",
    "$env:ProgramData\DellPair",
    "$env:ALLUSERSPROFILE\Dell\Dell Pair",
    "$env:ALLUSERSPROFILE\Dell Pair",
    "$env:ALLUSERSPROFILE\DellPair"
)
...
foreach ($profile in $userProfiles) {
    $userPaths = @(
        "$($profile.Path)\AppData\Local\Dell\Dell Pair",
        "$($profile.Path)\AppData\Local\Dell Pair",
        "$($profile.Path)\AppData\Local\DellPair",
        "$($profile.Path)\AppData\Roaming\Dell\Dell Pair",
        "$($profile.Path)\AppData\Roaming\Dell Pair",
        "$($profile.Path)\AppData\Roaming\DellPair"
    )
...
Write-Host "Cleaning Dell Pair registry entries..." -ForegroundColor Yellow
$regPaths = @(
    "HKLM:\SOFTWARE\Dell\Dell Pair",
    "HKLM:\SOFTWARE\Dell\DellPair",
    "HKLM:\SOFTWARE\DellPair",
    "HKLM:\SOFTWARE\WOW6432Node\Dell\Dell Pair",
    "HKLM:\SOFTWARE\WOW6432Node\Dell\DellPair",
    "HKLM:\SOFTWARE\WOW6432Node\DellPair"
)
...
foreach ($profile in $userProfiles) {
    if ($profile.SID -and $profile.SID -ne "S-1-5-18" -and $profile.SID -ne "S-1-5-19" -and $profile.SID -ne "S-1-5-20") {
        ...
        try {
            ...
            $userDellPaths = @(
                "$userRegPath\SOFTWARE\Dell\Dell Pair",
                "$userRegPath\SOFTWARE\Dell\DellPair",
                "$userRegPath\SOFTWARE\DellPair"
            )
    ...
    try {
        ...
        if ($runKeys) {
            $runKeys.PSObject.Properties | Where-Object { 
                ...
                ($_.Value -like "*Dell*Pair*" -or $_.Name -like "*Dell*Pair*" -or $_.Name -like "*DellPair*")
            } | ForEach-Object {
...
Write-Host "Processing Dell Pair services..." -ForegroundColor Yellow
$services = Get-Service | Where-Object { 
    $_.DisplayName -like "*Dell Pair*" -or 
    $_.DisplayName -like "*DellPair*" -or
    $_.ServiceName -like "*Dell*Pair*" -or
    $_.ServiceName -like "*DellPair*"
}
...
try {
    Get-ScheduledTask | Where-Object { 
        $_.TaskName -like "*Dell*Pair*" -or 
        $_.TaskName -like "*DellPair*" -or
        $_.TaskPath -like "*Dell*Pair*" -or
        $_.TaskPath -like "*DellPair*"
    } | ForEach-Object {
...
Write-Host "`nDell Pair application removal process completed!" -ForegroundColor Green
...
$flagPath = "$env:TEMP\DellPair_Removal_Complete.txt"
"Dell Pair application removal completed at $(Get-Date)" | Out-File $flagPath -Force

Write-Host "Completion flag created at: $flagPath" -ForegroundColor Cyan

# Additional cleanup - Force remove any remaining Dell Pair entries
...
foreach ($path in $additionalPaths) {
    try {
        Get-ChildItem $path -ErrorAction SilentlyContinue | ForEach-Object {
            ...
            # Check for Dell Pair with various naming patterns
            if (($displayName -like "*Dell*" -and $displayName -like "*Pair*") -or
                ($displayName -like "*DellPair*") -or
                ($publisher -like "*Dell*" -and $displayName -like "*Pair*") -or
                ($_.PSChildName -like "*Dell*" -and $displayName -like "*Pair*")) {
